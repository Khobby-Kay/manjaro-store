<div class="mobile-money-payment-container">
    <div class="text-center">
        <h3>{{ translate('Mobile Money Wallet Recharge') }}</h3>
        <p>{{ translate('Recharge your wallet using Mobile Money') }}</p>
    </div>

    <div class="payment-form-container">
        <form id="mobile-money-wallet-form" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="remarks" name="remarks" 
                       value="Wallet Recharge - Order #{{ $order_id }}" readonly>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('mobile-money-wallet-form');
    ...
    form.addEventListener('submit', function(e) {
        ...
        fetch('{{ route("mobile_money.debit_wallet") }}', {
            ...
            body: JSON.stringify({
                ...
                remarks: 'Wallet Recharge - Order #{{ $order_id }}'
            })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                alert('{{ translate("Wallet recharged successfully!") }}');
